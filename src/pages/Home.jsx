...
import { getHabitsByUser, getProgressForDate, upsertProgress } from '../autoBarrell'
...
export function Home() {
  ...
  const toggleMutation = useMutation({
    mutationFn: async (habit) => {
      ...
      const baseXP = 10
      const deltaXp = newCompleted ? baseXP * (habit.difficulty || 1) : -(existing?.xp_awarded || 0)
  ...
  return (
    <LinearGradient colors={['#667eea', '#764ba2', '#f093fb', '#4facfe']} style={styles.container} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      <View style={styles.header}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{profile?.display_name?.[0]?.toUpperCase() || 'U'}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={styles.welcome}>Hola, {profile?.display_name || user?.email}</Text>
      ...
      <View style={styles.card}>
        <View style={styles.progressBarContainer}>
          <LinearGradient colors={['#4facfe', '#00f2fe', '#43e97b']} style={styles.progressFill} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} />
        </View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 48, height: 48, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.9)', alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontWeight: '700', color: '#1F2937' },
  welcome: { color: '#fff', fontSize: 16, fontWeight: '600' },
  xpBarWrapper: { height: 8, marginTop: 6, borderRadius: 6, overflow: 'hidden' },
  xpBarBg: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, backgroundColor: 'rgba(255,255,255,0.3)' },
  ...
  card: { backgroundColor: '#fff', borderRadius: 24, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.2, shadowRadius: 16, elevation: 6 },
  progressBarContainer: { position: 'absolute', top: 0, left: 0, right: 0, height: 4, backgroundColor: '#E5E7EB', borderTopLeftRadius: 24, borderTopRightRadius: 24 },
