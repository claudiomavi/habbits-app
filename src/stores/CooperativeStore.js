...
import {
	...
	listMyInvitations,
	markOwnerNotificationSeen,
	rejectInvitation as rejectInv,
	subscribeInvitations,
	subscribeInvitationsByGroup,
...
export const useCooperativeStore = create((set, get) => ({
	...
	startOwnerNotificationsRealtime: async (user_id) => {
		...
		if (!user_id) return []
		const owned = (await listGroupsForUser(user_id)).filter(
			(g) => g.owner_id === user_id
		)
		const groupIds = owned.map((g) => g.id)
		const unsubs = groupIds.map((gid) =>
			subscribeInvitationsByGroup(gid, async () => {
	...
	// One-shot refresh for all cooperative data
	...
	fetchOwnerNotifications: async (user_id) => {
		...
		try {
			...
			const groupIds = owned.map((g) => g.id)
			if (!groupIds.length) {
				set({ notificationsOwner: [] })
				return []
			}
			// We only want acted ones (accepted/rejected), not pending, and only unseen
			const filtered = await listInvitationsByGroups({
				group_ids: groupIds,
				onlyUnseenByOwner: true,
			})
			set({
				notificationsOwner: (filtered || []).filter(
	...
	dismissOwnerNotification: async (id) => {
		...
		try {
			...
			console.warn(
				'markOwnerNotificationSeen failed, falling back to local dismiss',
				e
			)
		...
		set({
			notificationsOwner: get().notificationsOwner.filter((n) => n.id !== id),
		})
	},
