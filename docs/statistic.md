...
- Navegación:
  - Añadir tab “Estadísticas” al AppStack/TabNavigator.
  - Crear página Statistics.jsx + template StatisticsTemplate.jsx.
- Datos y helpers:
  - Hooks react-query para:
    - getUserHabits(userId)
    - getProgressRange(userId, from, to) (reutilizando endpoints existentes con un rango)
  - Helpers de cálculo (utils/stats):
    - buildScheduleMap(habits, from, to) → “qué días estaban programados por hábito”
    - computeStreakCurrentAndMax(habit, progress, from, to)
    - computeCompliance(habit, progress, from, to) → {programados, completados, pct}
    - computeGlobalAggregates(habits, progress, from, to)
  - Asegurar que todo usa fecha local YYYY-MM-DD (makeLocalISO) y monday-based para weekly; day_of_month para monthly.
